...
class RAGOrchestrator:
    ...
    async def _handle_play_music(self, corrected_query: str, entity: Optional[str], mode: str) -> dict:
        ...
        if not entity or entity.lower().strip() in generic_music_requests:
            return {
                ...
                "action_payload": {"placeholder": "เช่น Lover - Taylor Swift"},
                "sources": [],
                "image_url": None,
                "image_gallery": [],
            }
            ...
            if not search_results:
                return {
                    ...
                    "action": "PROMPT_FOR_SONG_INPUT",
                    "action_payload": {"placeholder": "ลองอีกครั้ง..."},
                    "sources": [],
                    "image_url": None,
                    "image_gallery": [],
                }
    ...
    async def _handle_informational(
        ...
        for sub_q in sub_queries:
            ...
            qdrant_results = await self.qdrant_manager.search_similar(
                query_text=sub_q, top_k=settings.QDRANT_TOP_K
            )
        ...
        if not unique_search_ids and not priority_doc:
            ...
            return {
                "answer": "ขออภัยค่ะ ไม่พบข้อมูลที่เกี่ยวข้อง",
                "action": None,
                "sources": [],
                "image_url": fallback_image,
                "image_gallery": [fallback_image] if fallback_image else [],
            }
        ...
        retrieved_docs = (
            await asyncio.to_thread(self.mongo_manager.get_locations_by_ids, final_mongo_ids)
            if final_mongo_ids
            else []
        )
        if not retrieved_docs:
            ...
            return {
                "answer": "ขออภัยค่ะ พบข้อมูลแต่ดึงรายละเอียดไม่ได้",
                "action": None,
                "sources": [],
                "image_url": fallback_image,
                "image_gallery": [fallback_image] if fallback_image else [],
        ...
        docs_with_synthetic = await asyncio.to_thread(
            lambda docs: [(doc, create_synthetic_document(doc)) for doc in docs],
            retrieved_docs,
        )
        ...
        topics_str_for_query = ", ".join(unique_topics) if unique_topics else primary_topic_fallback

        if primary_doc and self.log_collection is not None:
            try:
                log_document = {
                    "query": corrected_query,
                    "primary_topic": primary_topic_fallback,
        ...
        if len(static_image_gallery) < settings.IMAGE_FALLBACK_THRESHOLD:
            logging.warning(
                f"⚠️ Static images insufficient (found {len(static_image_gallery)}). Falling back to Google Search."
            )
            ...
            google_images = await image_search_tool_instance.get_image_urls(
                query=smarter_query, max_results=settings.GOOGLE_IMAGE_MAX_RESULTS
            )
    ...
    async def get_navigation_list(self) -> List[Dict[str, Any]]:
        ...
        query_filter = {
            "$or": [
                ...
                {"category": "ข้อมูลท่องเที่ยวเชิงธรรมชาติ"},
                {"category": "ข้อมูลท่องเที่ยวเชิงผจญภัย"},
                {"category": "ข้อมูลท่องเที่ยวเชิงธรรมชาติและทิวทัศน์"},
                {"category": "ข้อมูลร้านอาหารและแหล่งของกิน"},
                {"category": "ข้อมูลที่พักและโรงแรม"},
                {"category": "ข้อมูลแหล่งช้อปปิ้งและสินค้าที่ระลึก"},
                {"category": "ข้อมูลแหล่งช้อปปิ้งและบริการ"},
                {"category": "ข้อมูลร้านอาหารและที่พัก"},
                {"category": "ข้อมูลร้านอาหารและแหล่งช้อปปิ้ง"},
                {"category": "ข้อมูลสถานที่ท่องเที่ยวทางธรรมชาติ"},
                {"category": "ข้อมูลสถานที่ท่องเที่ยวทางธรรมชาติและชุมชน"},
                {"category": "ข้อมูลสถานที่ท่องเที่ยวทางวัฒนธรรมและศาสนา"},
                {"category": "ข้อมูลสถานที่ท่องเที่ยวและตลาด"},
                {"category": "ข้อมูลสถานที่ท่องเที่ยวทางวัฒนธรรมและประวัติศาสตร์"},
                {"category": "ข้อมูลสถานที่ท่องเที่ยวเชิงวิถีชีวิตและภูมิปัญญา"},
            ]
        }
        ...
        try:
            ...
            for doc in results_from_db:
                ...
                if doc.get("metadata") and doc["metadata"]:
                    prefix = doc["metadata"]
                    image_url = self._find_first_image_for_prefix(prefix)
    ...
    async def handle_get_directions(self, entity_slug: str, user_lat: float, user_lon: float) -> dict:
        ...
        if not api_key:
            ...
            return {
                "answer": "ขออภัยค่ะ ระบบแผนที่ยังไม่ได้ตั้งค่า API Key ค่ะ",
                "action": None,
                "sources": [],
                "image_url": None,
                "image_gallery": [],
        ...
        if not doc or not doc.get("navigation_data"):
            ...
            return {
                "answer": f"ขออภัยค่ะ น้องน่านยังไม่มีพิกัดแผนที่ของสถานที่นี้เลยค่ะ",
                "action": None,
                "sources": [],
                "image_url": None,
                "image_gallery": [],
        ...
        try:
            ...
            return {
                ...
                "action_payload": {"embed_url": embed_url, "destination_name": doc_title},
                "image_url": None,
                "image_gallery": [],
                "sources": [],
            }
            ...
            return {
                "answer": "ขออภัยค่ะ เกิดข้อผิดพลาดในการสร้าง URL แผนที่ค่ะ",
                "action": None,
                "sources": [],
                "image_url": None,
                "image_gallery": [],
    ...
    async def answer_query(self, query: str, mode: str = "text") -> dict:
        try:
            ...
            logging.error(
                f"❌ [Router V6] CRITICAL: Failed to interpret query: {e}. Defaulting to INFORMATIONAL.",
                exc_info=True,
            )
            fallback_image = self._find_any_random_image()
            return {
                "answer": "ขออภัยค่ะ มีปัญหาในการตีความคำถาม",
                "action": None,
                "sources": [],
                "image_url": fallback_image,
                "image_gallery": [fallback_image] if fallback_image else [],
        ...
        logging.info(
            f"🚦 [Router V6] Intent: {intent} | Entity: {entity} | "
        ...
        try:
            ...
            fallback_image = self._find_any_random_image()
            return {
                "answer": "ขออภัยค่ะ เกิดข้อผิดพลาดภายใน",
                "action": None,
                "sources": [],
                "image_url": fallback_image,
                "image_gallery": [fallback_image] if fallback_image else [],
