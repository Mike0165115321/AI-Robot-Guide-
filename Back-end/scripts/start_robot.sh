#!/bin/bash
# ===================================================================================
# 🤖 start_robot.sh - Robot System Launcher (Wrapper สำหรับ Launch File ใหม่)
# ===================================================================================
# ไฟล์นี้เป็น Wrapper สำหรับเรียกใช้ robot_system.launch.py
# สามารถใช้งานผ่าน Terminal หรือผ่าน HardwareService ได้
#
# การใช้งาน:
#   ./start_robot.sh         # เริ่มระบบทั้งหมด
#   Ctrl+C                   # หยุดระบบทั้งหมด (Clean Shutdown)
# ===================================================================================

BLUE='\033[0;34m'
GREEN='\033[0;32m'
RED='\033[0;31m'
NC='\033[0m'

echo -e "${BLUE}🤖 Starting Full Robot System...${NC}"

# =================================================================================
# 1. Environment Setup
# =================================================================================
source /opt/ros/humble/setup.bash
source /home/robot22/microros_ws/install/setup.bash
source /home/robot22/ros2robot/install/setup.bash

# กำหนด Path (ต้องกำหนดก่อนใช้งาน)
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BACKEND_ROOT="$(dirname "$SCRIPT_DIR")"

# =================================================================================
# 2. Cleanup ค้าง (กันกรณี process เก่าไม่ยอมตาย)
# =================================================================================
echo -e "${GREEN}🧹 กำลังล้าง process เก่าที่อาจค้างอยู่...${NC}"
pkill -f "micro_ros_agent" 2>/dev/null
pkill -f "ros2_bridge.py" 2>/dev/null
pkill -f "qos_relay_node.py" 2>/dev/null
pkill -f "scan_relay.py" 2>/dev/null
sleep 1

# =================================================================================
# 2.5 Reset ESP32/Teensy (บังคับให้มัน reconnect กับ Micro-ROS Agent)
# =================================================================================
echo -e "${GREEN}🔄 กำลัง Reset ESP32/Teensy...${NC}"
ESP32_RESET_SCRIPT="$BACKEND_ROOT/core/hardware/esp32_reset.py"
if [ -f "$ESP32_RESET_SCRIPT" ]; then
    python3 "$ESP32_RESET_SCRIPT" /dev/ttyACM0
else
    echo -e "${RED}⚠️ ไม่พบ esp32_reset.py, ใช้ sleep แทน...${NC}"
    sleep 2
fi


# =================================================================================
# 3. กำหนด Launch File Path
# =================================================================================
LAUNCH_FILE="$BACKEND_ROOT/core/hardware/robot_system.launch.py"

# =================================================================================
# 4. ตรวจสอบไฟล์ Launch
# =================================================================================
if [ ! -f "$LAUNCH_FILE" ]; then
    echo -e "${RED}❌ ไม่พบไฟล์ Launch: $LAUNCH_FILE${NC}"
    exit 1
fi

# =================================================================================
# 5. เริ่มระบบ
# =================================================================================
echo -e "${GREEN}🚀 กำลังเปิด Robot System Launch...${NC}"
echo -e "   Launch File: $LAUNCH_FILE"

# ros2 launch จะจัดการ lifecycle ของทุก node ให้
# เมื่อ script นี้ถูก kill หรือ Ctrl+C, ros2 launch จะปิด node ลูกทั้งหมดให้อัตโนมัติ
ros2 launch "$LAUNCH_FILE"

# Script จะรอที่นี่จนกว่า ros2 launch จะจบ (Ctrl+C หรือ error)
echo -e "${GREEN}✅ Robot System หยุดทำงานเรียบร้อย${NC}"
