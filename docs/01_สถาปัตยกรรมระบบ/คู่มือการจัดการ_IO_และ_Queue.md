...
sequenceDiagram
    ...
    Webhook->>Redis: 1. โยนงานเข้าคิว (lpush) ⚡️ เร็วมาก
    Webhook-->>User: ตอบกลับ "OK" (200) ทันที
    
    loop Background Process
        Worker->>Redis: 2. รอรับงาน (brpop)
        Redis-->>Worker: ได้งาน (JSON)
        Worker->>Worker: 3. ประมวลผล (AI / DB) 🐢 ช้าได้ตามสบาย
        Worker->>User: 4. ส่งข้อความตอบกลับ (Push Message)
    end
```
