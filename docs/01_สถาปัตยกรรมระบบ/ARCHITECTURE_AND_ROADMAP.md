...
## 🏛️ 1. สถาปัตยกรรมปัจจุบัน (Current Architecture)
...
```mermaid
graph TD
    User[🧔 User / Browser] -->|HTTP/WebSocket :8080| Gateway[🐹 Go Gateway]
    
    subgraph "LocalHOST (Internal Network)"
        Gateway -->|Proxy API/WS :9090| Python[🐍 Python Backend]
        Gateway -->|Serve Static Files| Frontend[📂 Frontend Assets]
        Gateway -.->|Future gRPC :50051| Cpp[🔊 C++ Audio Engine]
        
        Python -->|DB Query| Mongo[(🍃 MongoDB)]
        Python -->|Vector Search| Qdrant[(🎯 Qdrant)]
        Python -->|Edges| External[☁️ External APIs (Gemini/Groq)]
    end
```
